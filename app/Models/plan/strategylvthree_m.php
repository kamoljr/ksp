...
class Strategylvthree_m extends Model
{
  ...
  public function get_data()
  {
    ...
    $search_strategylvthree = $_POST["search_strategylvthree"];
    ...
    //---------แก้ sql----------
    $query = "SELECT strategylvthree_id,strategylvthree

    FROM  dbo.str_strategylvthree
         
    WHERE (del_item = N'0')";

    if ($search_strategylvtwo_id != "") {
       $query .= "  and (strategylvtwo_id = '$search_strategylvtwo_id')";
    }
  ...
  public function edit_data()
  {
    ...
    $sql = "Select strategylvthree_id,strategylvthree From str_strategylvthree where strategylvthree_id = " . $ids;
    $result = $this->db->query($sql);
    $data = $result->getResultArray();
    $resData = json_encode($data);
    return $resData;
  ...
  public function save_data()
  {
    ...
    $strategylvthree_add = $this->shared_m->replace_str($_POST["strategylvthree_add"]);
    ...
    if ($ids == '') {
      ...
      $sql = "INSERT INTO str_strategylvthree (strategylvtwo_id,strategylvthree,make_id,make_date,make_date_mktime) VALUES ($search_strategylvtwo_id,'$strategylvthree_add',$make_id,$make_date,'$make_date_mktime')";
      //echo $sql;
      ...
      $result = $this->db->query($sql);
        ...
        //------------------log เพิ่ม---------------------
      $max_ids = $this->shared_m->find_max_id("str_strategylvthree","strategylvthree_id");//แก่

      $action_detail = "เพิ่ม ".$_POST["for_log_app_name"]." : ".$strategylvthree_add."<br>";//แก่
      ...
      $sql = "Update str_strategylvthree set strategylvthree = '$strategylvthree_add',modify_id=$make_id,modify_date=$make_date,modify_date_mktime='$make_date_mktime' Where strategylvthree_id = $ids";
      //echo $sql;
      $result = $this->db->query($sql);
      //------------------log แก้ไข---------------------
      $max_ids = $ids;
      
      $action_detail = "แก้ไข ".$_POST["for_log_app_name"]." : ".$strategylvthree_add."<br>";//แก้
      //$action_detail = $action_detail."test : ".$unit_name;
  ...
  public function chk_del()
  {
    $ids = $_POST["ids"];
     ...
     $sql = "Select count(plantargetlvthree_id) as count_reccord From str_plantargetlvthree where del_item = '0' and strategylvthree_id = " . $ids;
    // echo $sql;
    $result = $this->db->query($sql);
    $data = $result->getResultArray();
    $resData = json_encode($data);
  ...
  public function del_data()
  {
    ...
    if ($count_reccord>0){
      ...
      $ids = $_POST["ids"];
      $sql = "Update str_strategylvthree set del_item = '1',delete_id=$make_id,delete_date=$make_date Where strategylvthree_id = ".$ids;
      //echo $sql;
      $result = $this->db->query($sql);
      ...
      $sql = "SELECT strategylvthree as aname FROM str_strategylvthree where strategylvthree_id = ".$ids;// แก้
      //echo $sql;
