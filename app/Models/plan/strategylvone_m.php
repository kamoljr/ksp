...
class Strategylvone_m extends Model
{
  ...
  public function get_data()
  {
    ...
    //---------ใส่ค่า----------
    $search_strategylvone = $_POST["search_strategylvone"];
    ...
    //---------แก้ sql----------
    $query = "SELECT strategylvone_id,strategylvone

    FROM  str_strategylvone 

    WHERE (del_item = N'0')";

    
    if ($search_strategylvone != "") {
       $query .= "  and (strategylvone like '%$search_strategylvone%' )";
    }
     //-----------------------
    //echo $query;
  ...
  public function edit_data()
  {
    ...
    $sql = "Select strategylvone_id,strategylvone From str_strategylvone where strategylvone_id = " . $ids;
    $result = $this->db->query($sql);
    $data = $result->getResultArray();
    $resData = json_encode($data);
    return $resData;
  ...
  public function save_data()
  {
    ...
    $strategylvone_add = $this->shared_m->replace_str($_POST["strategylvone_add"]);
    ...
    $make_id = $_SESSION['user_id'];
      ...
      if ($ids == '') {
        ...
        $sql = "INSERT INTO str_strategylvone (strategylvone,make_id,make_date,make_date_mktime) VALUES ('$strategylvone_add',$make_id,$make_date,'$make_date_mktime')";
        //echo $sql;
        $result = $this->db->query($sql);
       
         //------------------log เพิ่ม---------------------
        $max_ids = $this->shared_m->find_max_id("str_strategylvone","strategylvone_id");//แก่

        $action_detail = "เพิ่ม ".$_POST["for_log_app_name"]." : ".$strategylvone_add."<br>";//แก่
        ...
        $sql = "Update str_strategylvone set strategylvone = '$strategylvone_add',modify_id=$make_id,modify_date=$make_date,modify_date_mktime='$make_date_mktime' Where strategylvone_id = $ids";
        //echo $sql;
        $result = $this->db->query($sql);
        //------------------log แก้ไข---------------------
        $max_ids = $ids;
        
        $action_detail = "แก้ไข ".$_POST["for_log_app_name"]." : ".$strategylvone_add."<br>";//แก้
        //$action_detail = $action_detail."test : ".$unit_name;
  ...
  public function chk_del()
  {
    ...
    $sql = "Select count(plantargetlvone_id) as count_reccord From str_plantargetlvone where del_item = '0' and strategylvone_id = " . $ids;
    // echo $sql;
    $result = $this->db->query($sql);
    $data = $result->getResultArray();
    $resData = json_encode($data);
  ...
  public function del_data()
  {
    ...
    if ($count_reccord>0){
      ...
      $ids = $_POST["ids"];
      $sql = "Update str_strategylvone set del_item = '1',delete_id=$make_id,delete_date=$make_date Where strategylvone_id = ".$ids;
      //echo $sql;
      $result = $this->db->query($sql);
      ...
      $sql = "SELECT strategylvone as aname FROM str_strategylvone where strategylvone_id = ".$ids;// แก้
      //echo $sql;
