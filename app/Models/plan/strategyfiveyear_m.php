...
class Strategyfiveyear_m extends Model
{
  ...
  public function get_data()
  {
    ...
    //---------แก้ sql----------
    $query = "SELECT strategylvfour_id,start_year,end_year,vision

    FROM  dbo.str_strategylvfour
         
    WHERE (del_item = N'0')";

    if ($search_strategylvthree_id != "") {
       $query .= "  and (strategylvthree_id = '$search_strategylvthree_id')";
    }
  ...
  public function edit_data()
  {
    ...
    $sql = "Select start_year,end_year,vision,popularity,achievement,achievement_ind,product From str_strategylvfour where strategylvfour_id = " . $ids;
    $result = $this->db->query($sql);
    $data = $result->getResultArray();
    $resData = json_encode($data);
    return $resData;
  ...
  public function save_data()
  {
    ...
    $start_year_add = $this->shared_m->replace_str($_POST["start_year_add"]);
    $end_year_add = intval($start_year_add) + 4;
    ...
    if ($ids == '') {
      ...
      $sql = "INSERT INTO str_strategylvfour (strategylvthree_id,start_year,end_year,vision,popularity,achievement,achievement_ind,product,make_id,make_date,make_date_mktime) VALUES ($search_strategylvthree_id,'$start_year_add','$end_year_add','$vision_add','$popularity_add','$achievement_add','$achievement_ind_add','$product_add',$make_id,$make_date,'$make_date_mktime')";
      //echo $sql;
      ...
      $sql = "Update str_strategylvfour set start_year = '$start_year_add',end_year = '$end_year_add',vision = '$vision_add',popularity = '$popularity_add',achievement = '$achievement_add',achievement_ind = '$achievement_ind_add',product = '$product_add',modify_id=$make_id,modify_date=$make_date,modify_date_mktime='$make_date_mktime' Where strategylvfour_id = $ids";
      //echo $sql;
      $result = $this->db->query($sql);
      //------------------log แก้ไข---------------------
      $max_ids = $ids;
      
      $action_detail = "แก้ไข ".$_POST["for_log_app_name"]." : ".$vision_add."<br>";//แก้
      //$action_detail = $action_detail."test : ".$unit_name;
  ...
  public function chk_del()
  {
    $ids = $_POST["ids"];
     ...
     $sql = "Select count(plantargetlvfour_id) as count_reccord From str_plantargetlvfour where del_item = '0' and strategylvfour_id = " . $ids;
    // echo $sql;
    $result = $this->db->query($sql);
    $data = $result->getResultArray();
    $resData = json_encode($data);
  ...
  public function del_data()
  {
    ...
    if ($count_reccord>0){
      ...
      $ids = $_POST["ids"];
      $sql = "Update str_strategylvfour set del_item = '1',delete_id=$make_id,delete_date=$make_date Where strategylvfour_id = ".$ids;
      //echo $sql;
      $result = $this->db->query($sql);
      ...
      $sql = "SELECT vision as aname FROM str_strategylvfour where strategylvfour_id = ".$ids;// แก้
      //echo $sql;
