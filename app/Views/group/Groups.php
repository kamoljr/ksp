                  ...
                  <form action="<?=$actual_link?>" method="post" accept-charset="utf-8" id="form-search" name="form-search">
                    ...
                    <div class="row">
                      ...
                      <div class="col-sm">
                        ...
                        <input id="unit_name_search" name="unit_name_search" class="form-control searchdata" type="text" placeholder="ชื่อส่วนงาน"/>
                      </div>
...
<script src="<?= base_url().'/public/';?>js/budget/<?=$pages;?>.js"></script>
<script>
<?
$displaymode = '';
...
$(document).ready(function() {
  ...
  iDisplayLength: 8,
  bServerSide: true,
  sAjaxSource: "/<?=$pages?>_cn/load_data",
  async: false,
  aaSorting: [],
  ...
  $('#dynamic-table tbody').on('click', 'td .edit-data', function () {
    ...
    if (displaymode == 'inline'){
        ...
        $('#dialogmode').html(`<?=view("{$pages}/{$pages}add.php")?>`);
  ...
  function format ( d ) {
      $(".dialog-success").hide();
      return `<?=view("{$pages}/{$pages}add.php")?>`;
  }
  ...
  $(document).on('click', '#btnadd', function(){
      ...
      table.draw();
      if (displaymode == 'inline'){
          $('#collapsedivadd').html(`<?=view("{$pages}/{$pages}add.php")?>`);
