          ...
          <table id="dynamic-table" class="table table-md table-bordered table-hover table-striped w-100">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  ผู้ประกอบวิชาชีพครูและบุคลากรทางการศึกษามีคุณภาพตามมาตรฐานวิชาชีพ
                </td>
                ...
                str_btn = "โครงการระดับผลผลิต";
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.product_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
	...
	$("#form_save").validate({
		rules: {
			...
			plan_id_add: "required",
			
		},
		messages: {
			product_add: "กรุณาป้อน โครงการ/ผลผลิต",
			plan_id_add: "กรุณาป้อน ด้าน",
...
// ------------------open แกไข จุดที่ 2/2-----------------------
    ...
    // $( "#plantargetlvfour_id_multi_3" ).prop( "checked", true );
		...
		$("#ids").val(d.product_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData1 = "ids=" + d.product_id;
		$.ajax({
			type: "POST",
			...
			success: function (response) {
				...
				$("#activity_add").val(response[0].activity);
				
        if (response[0].master_plan_id != null){
          let text = response[0].master_plan_id;
          let text1 = response[0].product_type;
          setTimeout(function () {
              $("#master_plan_id_"+text).attr('checked', 'checked');
							$("#product_type"+text1).attr('checked', 'checked');
							$("#product_type"+text1).prop('checked', true);
		...
		$('#lbl_rowid').text("รหัส " + d.product_id);
