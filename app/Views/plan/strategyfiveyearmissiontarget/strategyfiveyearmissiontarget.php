          ...
          <table id="dynamic-table" class="table table-md table-bordered table-hover table-striped w-100">
            ...
            <tbody>
              <!-- <tr>
                ...
                <td>
                  ครูและบุคลากรทางการศึกษาได้รับการพัฒนาอย่างเป็นระบบ
                </td>
			...
			columns: [
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.mission_target_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
	...
	$(document).on('click', '#btnadd', function() {
		...
		//-------open------
    if ($("#search_strategylvfour_id").val() == ""){
      $('#plantargetlvfour_id_add').empty()
      $('#plantargetlvfour_id_add').append('<option selected="selected" value="">ไม่พบข้อมูล</option>');
    }else{
      const select_var10 = {
        ele_name:"plantargetlvfour_id_add",
        value_edit:"",
        option_type:"",
        placeholder:"แผนปฏิบัติการระยะ 5 ปี",
...
// ------------------open แกไข จุดที่ 2/2-----------------------
	function editdata(d, mode) {
		...
		$("#ids").val(d.mission_target_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData = "ids=" + d.mission_target_id;
		$.ajax({
			type: "POST",
          ...
          if ($("#search_strategylvfour_id").val() == ""){
            ...
            const select_var10 = {
              ele_name:"plantargetlvfour_id_add",
              value_edit:response[0].plantargetlvfour_id,
              option_type:"",
              placeholder:"แผนปฏิบัติการระยะ 5 ปี",
              classselect:"ele_select_add",
              ctr1:$("#search_strategylvfour_id").val(),
          ...
          //--------close-----
				$("#mission_target_add").val(response[0].mission_target);
				$("#mission_target_comment_add").val(response[0].mission_target_comment);
		...
		$('#lbl_rowid').text("รหัส " + d.mission_target_id);
