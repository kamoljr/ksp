          ...
          <table id="dynamic-table" class="table table-md table-bordered table-hover table-striped w-100">
            ...
            <tbody>
              <!-- <tr>
                ...
                <td>
                  คุรุสภาเป็นองค์กรวิชาชีพที่มีระบบและกลไก ในการ ส่งเสริม สนับสนุน เชื่อมโยง การผลิต การคัดกรอง และพัฒนาผู้ประกอบวิชาชีพครูและบุคลากรทางการศึกษาที่มีคุณภาพได้มาตรฐานสากล	
                </td>
                <td>
                  2566
                </td>
			...
			columns: [
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.strategyyear_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
    ...
    // load default แผนระยะ 5 ปี
    let aoData = "ids=" + $("#search_strategylvfour_id").val();
		$.ajax({
			type: "POST",
			url: "/public/index.php/<?=$description_en.'/'.$pages?>_cn/load_default",
			dataType: "json",
			beforeSend: function () {
				$("#overlay").fadeIn(200);
			},
			...
			success: function (response) {
				...
				$("#vision_add").val(response[0].vision);
				$("#popularity_add").val(response[0].popularity);
				
				$("#achievement_add").val(response[0].achievement);
				$("#achievement_ind_add").val(response[0].achievement_ind);
			},
			complete: function () {
				setTimeout(function () {
					$("#overlay").fadeOut(200);
	...
	$("#form_save").validate({
		rules: {
			budget_year_add: "required",
			vision_add: "required",
      popularity_add: "required",
			target_add: "required",
			target_ind_add: "required",
			achievement_add: "required",
			achievement_ind_add: "required",
		},
		messages: {
			budget_year_add: "กรุณาป้อน ปีงบประมาณ",
			vision_add: "กรุณาป้อน วิสัยทัศน์",
			popularity_add: "กรุณาป้อน ค่านิยม",
			target_add: "กรุณาป้อน เป้าหมายการให้บริการประทรวง",
			target_ind_add: "กรุณาป้อน ตัวชี้วัดเป้าหมายการให้บริการประทรวง",
			achievement_add: "กรุณาป้อน ผลสัมฤทธิ์และประโยชน์ที่คาดว่าจะได้รับ",
			achievement_ind_add: "กรุณาป้อน ตัวชี้วัดผลสัมฤทธิ์ ",
		},
...
// ------------------open แกไข จุดที่ 2/2-----------------------
	function editdata(d, mode) {
		...
		$("#ids").val(d.strategyyear_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData = "ids=" + d.strategyyear_id;
		$.ajax({
			type: "POST",
			...
			success: function (response) {
				...
				$("#budget_year_add").val(response[0].budget_year);
				$("#vision_add").val(response[0].vision);
				$("#popularity_add").val(response[0].popularity);
				$("#target_add").val(response[0].target);
				$("#target_ind_add").val(response[0].target_ind);
				$("#achievement_add").val(response[0].achievement);
				$("#achievement_ind_add").val(response[0].achievement_ind);
			},
			complete: function () {
		...
		$('#lbl_rowid').text("รหัส " + d.strategyyear_id);
