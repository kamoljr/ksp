                ...
                id="form-search" name="form-search">
                  <div class="row">
                    ...
                    <div class="col-sm-3">
                      ...
                      <select id="search_swot" name="search_swot" class="select2 searchdata" placeholder="SWOT">
                        <option value="s">
                          Strengths
                        </option>
                        <option value="w">
                    ...
                    <div class="col-sm-6">
                      ...
                      <input id="search_issues" name="search_issues" class="form-control searchdata" type="text" placeholder="ประเด็น">
                    </div>
  ...
  // });
	...
	var table = $("#dynamic-table").DataTable({
			...
			//	เปิด แก้ไข ใส่ค่า ตัวค้นหา
				...
				aoData.push({
						name: 'search_swot',
						value: $("#search_swot").val(),
				});
				aoData.push({
						name: 'search_issues',
						value: $("#search_issues").val(),
				});
			...
			columns: [
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.issuesandswot_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
	...
	$(document).on('click', '#btnadd', function() {
		...
		$("#form_add_lbl_swot").text($("#search_swot option:selected" ).text());
	...
	function searchdata(){
		...
		text2 = "SWOT : ";
		sval2 = $("#search_swot option:selected" ).text();
		if (sval2 == '') { sval2 = 'ทั้งหมด'; }
		text2 = text2+"<b>"+sval2+"</b>";

		text3 = "ประเด็น : ";
		sval3 = $("#search_issues option:selected" ).text();
...
// ------------------open แกไข จุดที่ 2/2-----------------------
    ...
    // $( "#plantargetlvfour_id_multi_3" ).prop( "checked", true );
		...
		$("#ids").val(d.issuesandswot_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData = "ids=" + d.issuesandswot_id;
		$.ajax({
			type: "POST",
			...
			success: function (response) {
				...
				$("#issues_add").val(response[0].issues);
		...
		$('#lbl_rowid').text("รหัส " + d.issuesandswot_id);
...
function savedata() {
	...
	// -----ใส่ค่า--------
	 var aoData = $('#form_save').serialize()+"&search_strategyyear_id="+$("#search_strategyyear_id").val()+"&search_swot="+$("#search_swot").val();
