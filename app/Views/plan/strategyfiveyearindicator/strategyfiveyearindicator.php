                ...
                id="form-search" name="form-search">
                  <div class="row">
                    ...
                    <div class="col-sm-6">
                      ...
                      <div id = "div_search_plantargetlvfour_id"></div>
                    </div>
			...
			columns: [
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.indicatorlvfour_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
	...
	$("#form_save").validate({
		rules: {
			indicatorlvthree_id_add: "required",
			indicatorlvfour_add: "required",
			indicatorlvfour_unit_add: "required",
		},
		messages: {
			indicatorlvthree_id_add: "กรุณาป้อน เชื่อมโยงตัวชี้วัดแผนระดับ3",
			indicatorlvfour_add: "กรุณาป้อน ตัวชี้วัดแผนระดับ4",
			indicatorlvfour_unit_add: "กรุณาป้อน หน่วย ",
		},
...
// ------------------open แกไข จุดที่ 2/2-----------------------
	function editdata(d, mode) {
		...
		$("#ids").val(d.indicatorlvfour_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData = "ids=" + d.indicatorlvfour_id;
		$.ajax({
			type: "POST",
        ...
        //--------close-----
				...
				$("#indicatorlvfour_add").val(response[0].indicatorlvfour);
				$("#indicatorlvfour_unit_add").val(response[0].indicatorlvfour_unit);
		...
		$('#lbl_rowid').text("รหัส " + d.indicatorlvfour_id);
...
function savedata() {
	...
	// -----ใส่ค่า--------
	var aoData = $('#form_save').serialize()+"&search_strategylvfour_id="+$("#search_strategylvfour_id").val()+"&search_plantargetlvfour_id="+$("#search_plantargetlvfour_id").val();
	//----------------
	console.log(aoData);
	$.ajax({
		type: "POST",
