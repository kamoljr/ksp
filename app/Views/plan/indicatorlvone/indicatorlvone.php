...
<div class="row">
  <div class="col-xl-12">
              ...
              <div class="card-body">
                ...
                id="form-search" name="form-search">
                  <div class="row">
                    ...
                    <div class="col-sm-4">
                      <label class="form-label" for="search_plantargetlvone_id">
                        เป้าหมายแผนยุทธศาสตร์ชาติ
                      </label>
                      <div id = "div_search_plantargetlvone_id"></div>
                    </div>
                    <div class="col-sm-4">
                      <label class="form-label" for="search_indicatorlvone">
                         ตัวชี้วัดแผนยุทธศาสตร์ชาติ
                      </label>
                      <input id="search_indicatorlvone" name="search_indicatorlvone" class="form-control searchdata"
    ...
    style="margin-bottom: 15px;">
      <!-- ค้นหาโดย : แผนยุทธศาสตร์ชาติ ด้านการพัฒนาและเสริมสร้างศักยภาพทรัพยากรมนุยษ์ -->
    </div>
  </div>
</div>
<div class="row">
          ...
          <table id="dynamic-table" class="table table-md table-bordered table-hover table-striped w-100">
            ...
            <tbody>
              <!-- <tr>
                ...
                <td>
                  3.2 ผลสัมฤทธิ์ทางการศึกษาและการเรียนรู้ตลอดชีวิต
                </td>
...
$(document).ready(function() {
  ...
  //-------open------
	const select_var1 = {
		ele_name:"search_plantargetlvone_id", 
		value_edit:"",
		option_type:"",
		placeholder:"เป้าหมายแผนยุทธศาสตร์ชาติ",
		classselect:"select2 searchdata",
	...
	$(document).on('change', '#search_strategylvone_id', function() {
		const select_var1 = {
			ele_name:"search_plantargetlvone_id", // ชื่อ element
	...
	var table = $("#dynamic-table").DataTable({
			...
			fnServerParams: function(aoData) {
				...
				aoData.push({
						name: 'search_plantargetlvone_id',
						value: $("#search_plantargetlvone_id").val(),
				});
				aoData.push({
						name: 'search_indicatorlvone',
						value: $("#search_indicatorlvone").val(),
				});
			...
			columns: [
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.indicatorlvone_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
	...
	$(document).on('click', '#btnadd', function() {
		...
		$("#form_add_lbl_plantargetlvone").text($("#search_plantargetlvone_id option:selected" ).text());
	...
	$("#form_save").validate({
		rules: {
			indicatorlvone_add: "required",
		},
		messages: {
			indicatorlvone_add: "กรุณาป้อน ตัวชี้วัดแผนยุทธศาสตร์ชาติ",
		},
	...
	function searchdata(){
		...
		text1 = "แผนยุทธศาสตร์ชาติ : ";
		sval1 = $("#search_strategylvone_id option:selected" ).text();
		if (sval1 == '') { sval1 = 'ทั้งหมด'; }
		text1 = text1+"<b>"+sval1+"</b>";

		text2 = "เป้าหมายแผนยุทธศาสตร์ชาติ : ";
		sval2 = $("#search_plantargetlvone_id option:selected" ).text();
		if (sval2 == '') { sval2 = 'ทั้งหมด'; }
		text2 = text2+"<b>"+sval2+"</b>";

		text3 = "ตัวชี้วัดแผนยุทธศาสตร์ชาติ : ";
		sval3 = $("#search_indicatorlvone" ).val();
...
// ------------------open แกไข จุดที่ 2/2-----------------------
	function editdata(d, mode) {
		...
		$("#ids").val(d.indicatorlvone_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData = "ids=" + d.indicatorlvone_id;
		$.ajax({
			type: "POST",
			...
			success: function (response) {
				...
				$("#indicatorlvone_add").val(response[0].indicatorlvone);
				$("#indicatorlvone_unit_add").val(response[0].indicatorlvone_unit);
		...
		$('#lbl_rowid').text("รหัส " + d.indicatorlvone_id);
...
function savedata() {
	...
	// -----ใส่ค่า--------
	var aoData = $('#form_save').serialize()+"&search_strategylvone_id="+$("#search_strategylvone_id").val()+"&search_plantargetlvone_id="+$("#search_plantargetlvone_id").val();
	//----------------
	console.log(aoData);
	$.ajax({
		type: "POST",
