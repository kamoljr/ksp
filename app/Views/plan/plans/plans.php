			...
			columns: [
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.plan_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
    ...
    let aoData = "ids=0";
        ...
        selected = '';
          ...
          str_select = str_select+'<input type="radio" class="custom-control-input" id="master_plan_id_'+response[i].field_id+'" name="master_plan_id" value = "'+response[i].field_id+'"'+selected+'>';
	...
	$("#form_save").validate({
		rules: {
			plan_name_add: "required",
			
		},
		messages: {
			plan_name_add: "กรุณาป้อน ด้าน",
    ...
    // $( "#plantargetlvfour_id_multi_3" ).prop( "checked", true );
		...
		$.ajax({
			...
			success: function (response) {
				...
				$("#plan_name_add").val(response[0].plan_name);
