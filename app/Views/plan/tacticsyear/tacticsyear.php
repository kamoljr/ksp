  ...
  // });
	...
	var table = $("#dynamic-table").DataTable({
			...
			columns: [
					...
					{
						data: "strategy_swot_id",
					},
					{
						data: "strategy_swot",
					},
					{
						data: "strategy_swot_type",
					},
					{
						data: "strategy_swot_id",
						render: function(data, type, row) {
							...
							// str_btn =
							// 	str_btn +
							// 	`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.strategy_swot_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
    ...
    let aoData = "ids=" + $("#search_strategyyear_id").val();
        ...
        str_select = str_select+'<div class="demo color-fusion-500">';
          ...
          str_select = str_select+'<input type="checkbox" class="custom-control-input" id="policyksp_id_multi_'+response[i].field_id+'" name="policyksp_id_multi[]" value = '+response[i].field_id+'>';
          str_select = str_select+'<label class="custom-control-label" for="policyksp_id_multi_'+response[i].field_id+'">'+response[i].field_name+'</label>';
        ...
        str_select = str_select+'</div>';
        str_select = str_select+'</div>';
        $("#div_policyksp_id_add").html(str_select);
			},
			complete: function () {
...
// ------------------open แกไข จุดที่ 2/2-----------------------
    ...
    // $('#policyksp_id_multi_3').is(':checked') ;
    // $( "#policyksp_id_multi_3" ).prop( "checked", true );
		...
		$("#ids").val(d.strategy_swot_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData = "ids=" + d.strategy_swot_id;
		$.ajax({
			type: "POST",
			...
			success: function (response) {
				...
				$("#strategy_swot_add").text(response[0].strategy_swot);
				
        //--- ติ๊ก checkbox----
        if (response[0].policyksp_id_multi != null){
          let text = response[0].policyksp_id_multi;
          const array1 = text.split(",");
          ii = 0;
          setTimeout(function () {
            while (ii < array1.length) {
              console.log(array1[ii]);
              $('#policyksp_id_multi_'+array1[ii]).prop('checked', true);
              ii++;
          }
          }, 100);
        }
		...
		$('#lbl_rowid').text("รหัส " + d.strategy_swot_id);
