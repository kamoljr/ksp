              ...
              <div class="card-body">
                ...
                id="form-search" name="form-search">
                  <div class="row">
                    ...
                    <div class="col-sm-4">
                      <label class="form-label" for="search_strategylvtwo">
                        แผนระดับ3
                      </label>
                      <input id="search_strategylvthree" name="search_strategylvthree" class="form-control searchdata"
                      type="text" placeholder="แผนระดับ3" />
                    </div>
                  </div>
                </form>
              </div>
                  ...
                  <div class="btn-group-vertical" role="group" aria-label="Vertical button group">
                    <div class="btn-group" role="group">
                        ...
                        <div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop1">
                            <a class="dropdown-item" href="plantargetlvtwo">เป้าหมาย</a>
                            <a class="dropdown-item" href="indicatorlvtwo">ตัวชี้วัด</a>
  ...
  //-------open------
	...
	var table = $("#dynamic-table").DataTable({
			...
			fnServerParams: function(aoData) {
				...
				aoData.push({
						name: 'search_strategylvthree',
						value: $("#search_strategylvthree").val(),
				});
			...
			columns: [
					...
					{
						data: "strategylvthree_id",
					},
					{
						data: "strategylvthree",
					},
					{
						data: "strategylvthree_id",
            render: function(data, type, row) {
                ...
                str_btn = str_btn+'<div class="btn-group" role="group">';
                    str_btn = str_btn+'<button id="btnGroupVerticalDrop'+row.strategylvthree_id+'" type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">';
                      str_btn = str_btn+'ไปยัง';
                    str_btn = str_btn+'</button>';
                    str_btn = str_btn+'<div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop1">';
                        str_btn = str_btn+'<a class="dropdown-item" href="plantargetlvtwo/'+row.strategylvthree_id+'/'+row.strategylvone_id+'">เป้าหมาย</a>';
                        str_btn = str_btn+'<a class="dropdown-item" href="indicatorlvtwo/'+row.strategylvthree_id+'/'+row.strategylvone_id+'">ตัวชี้วัด</a>';
              ...
              str_btn = str_btn+'</div>';
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.strategylvthree_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
	...
	$("#form_save").validate({
		rules: {
			strategylvthree_add: "required",
		},
		messages: {
			strategylvthree_add: "กรุณาป้อน แผนระดับ3",
		},
	...
	function searchdata(){
		...
		text3 = "แผนระดับ3 : ";
		sval3 = $("#search_strategylvthree" ).val();
...
// ------------------open แกไข จุดที่ 2/2-----------------------
	function editdata(d, mode) {
		...
		$("#ids").val(d.strategylvthree_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData = "ids=" + d.strategylvthree_id;
		$.ajax({
			type: "POST",
			...
			success: function (response) {
				...
				$("#strategylvthree_add").val(response[0].strategylvthree);
		...
		$('#lbl_rowid').text("รหัส " + d.strategylvthree_id);
...
function savedata() {
	...
	// -----ใส่ค่า--------
	var aoData = $('#form_save').serialize()+"&search_strategylvone_id="+$("#search_strategylvone_id").val()+"&search_strategylvtwo_id="+$("#search_strategylvtwo_id").val();
	//----------------
	console.log(aoData);
	$.ajax({
		type: "POST",
