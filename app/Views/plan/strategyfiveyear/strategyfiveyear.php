          ...
          <table id="dynamic-table" class="table table-md table-bordered table-hover table-striped w-100">
            ...
            <tbody>
              <!-- <tr>
                ...
                <td>
                  คุรุสภาเป็นองค์กรวิชาชีพที่มีระบบและกลไก ในการส่งเสริม สนับสนุน เชื่อมโยง การผลิต การคัดกรองและพัฒนาผู้ประกอบวิชาชีพครูและบุคลากรทางการศึกษาที่มีคุณภาพได้มาตรฐานสากล
                </td>
                <td>
                  2566-2570
                </td>
             ...
             render: function(data, type, row) {
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.strategylvfour_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------open แกไข จุดที่ 2/2-----------------------
	function editdata(d, mode) {
		...
		$("#ids").val(d.strategylvfour_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData = "ids=" + d.strategylvfour_id;
		$.ajax({
			type: "POST",
			...
			success: function (response) {
				...
				$("#start_year_add").val(response[0].start_year);
				...
				$("#product_add").val(response[0].product);
		...
		$('#lbl_rowid').text("รหัส " + d.strategylvfour_id);
...
function savedata() {
	...
	// -----ใส่ค่า--------
	var aoData = $('#form_save').serialize()+"&search_strategylvthree_id="+$("#search_strategylvthree_id").val();
	//----------------
	console.log(aoData);
	$.ajax({
		type: "POST",
