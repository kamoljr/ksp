			...
			columns: [
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.mission_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
	...
	$("#form_save").validate({
		rules: {
			mission_add: "required",
		},
		messages: {
			mission_add: "กรุณาป้อน พันธกิจ",
		},
...
// ------------------open แกไข จุดที่ 2/2-----------------------
	function editdata(d, mode) {
		...
		$("#ids").val(d.mission_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData = "ids=" + d.mission_id;
		$.ajax({
			type: "POST",
			...
			success: function (response) {
				...
				$("#mission_add").val(response[0].mission);
		...
		$('#lbl_rowid').text("รหัส " + d.mission_id);
