                ...
                id="form-search" name="form-search">
                  <div class="row">
                    ...
                    <div class="col-sm-6">
                      ...
                      <input id="search_strategylvtwo" name="search_strategylvtwo" class="form-control searchdata"
                      type="text" placeholder="แผนระดับ2" />
                    </div>
                  </div>
                </form>
              </div>
...
$(document).ready(function() {
	...
	var table = $("#dynamic-table").DataTable({
			...
			fnServerParams: function(aoData) {
				...
				// });
				aoData.push({
						name: 'search_strategylvtwo',
						value: $("#search_strategylvtwo").val(),
				});
            ...
            render: function(data, type, row) {
                ...
                str_btn = str_btn+'<div class="btn-group" role="group">';
                    str_btn = str_btn+'<button id="btnGroupVerticalDrop'+row.strategylvtwo_id+'" type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">';
                      str_btn = str_btn+'ไปยัง';
                    str_btn = str_btn+'</button>';
                    str_btn = str_btn+'<div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop1">';
                        str_btn = str_btn+'<a class="dropdown-item" href="plantargetlvtwo/'+row.strategylvtwo_id+'/'+row.strategylvone_id+'">เป้าหมาย</a>';
                        str_btn = str_btn+'<a class="dropdown-item" href="indicatorlvtwo/'+row.strategylvtwo_id+'/'+row.strategylvone_id+'">ตัวชี้วัด</a>';
              ...
              str_btn = str_btn+'</div>';
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.strategylvtwo_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
	...
	$("#form_save").validate({
		rules: {
			strategylvtwo_add: "required",
		},
		messages: {
			strategylvtwo_add: "กรุณาป้อน แผนระดับ2",
		},
	...
	function searchdata(){
		...
		text3 = "แผนระดับ2 : ";
		sval3 = $("#search_strategylvtwo" ).val();
...
// ------------------open แกไข จุดที่ 2/2-----------------------
	function editdata(d, mode) {
		...
		$("#ids").val(d.strategylvtwo_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData = "ids=" + d.strategylvtwo_id;
		$.ajax({
			type: "POST",
			...
			success: function (response) {
				...
				$("#strategylvtwo_add").val(response[0].strategylvtwo);
		...
		$('#lbl_rowid').text("รหัส " + d.strategylvtwo_id);
