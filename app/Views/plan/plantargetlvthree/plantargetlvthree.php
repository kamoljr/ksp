...
<div class="row">
  <div class="col-xl-12">
                ...
                id="form-search" name="form-search">
                  <div class="row">
                    ...
                    <div class="col-sm-3">
                      ...
                      <input id="search_plantargetlvthree" name="search_plantargetlvthree" class="form-control searchdata"
                      type="text" placeholder="เป้าหมายแผนระดับ3" />
                    </div>
                  </div>
                </form>
              </div>
    ...
    <div class="alert alert-warning" id='searchresults' name='searchresults' style="margin-bottom: 15px;">
      <!-- ค้นหาโดย : <u>แผนยุทธศาสตร์ชาติ </u>ด้านการพัฒนาและเสริมสร้างศักยภาพทรัพยากรมนุยษ์ <u>แผนระดับ3</u> ประเด็น 12 การพัฒนาการเรียนรู้ (พ.ศ. 2561-2580) -->
    </div>
  </div>
</div>
<div class="row">
			...
			columns: [
					...
					},
					{
						...
						render: function(data, type, row) {
							...
							str_btn =
								str_btn +
								`<a href="javascript:void(0);" class="btn btn-outline-danger btn-icon btn-xs rounded-circle waves-effect waves-themed"data-toggle="modal" data-target="#example-modal-alert" onclick=set_del_id('${row.plantargetlvthree_id}')><i class="ni ni-trash"></i></a>`;
							return str_btn;
						},
					},
...
// ------------------close ค้าหา จุดที่ 1/2-----------------------
	...
	$(document).on('click', '#btnadd', function() {
		...
		//-------open------
    const select_var10 = {
      ele_name:"plantargetlvtwo_id_add",
      value_edit:"",
      option_type:"",
      placeholder:"เชื่อมเป้าหมายแผนระดับ2",
...
// ------------------open แกไข จุดที่ 2/2-----------------------
	function editdata(d, mode) {
		...
		$("#ids").val(d.plantargetlvthree_id);
		
		// ปิด แก้ไข ใส่ค่า
		
		let aoData = "ids=" + d.plantargetlvthree_id;
		$.ajax({
			type: "POST",
			...
			success: function (response) {
				...
				const select_var11 = {
					ele_name:"plantargetlvtwo_id_add",
					value_edit:response[0].plantargetlvtwo_id,
					option_type:"",
					placeholder:"เชื่อมเป้าหมายแผนระดับ2",
					classselect:"ele_select_add",
					ctr1:$("#search_strategylvtwo_id").val(),
				...
				//--------close-----
				$("#plantargetlvthree_add").val(response[0].plantargetlvthree);
				$("#plantargetlvthree_comment_add").val(response[0].plantargetlvthree_comment);
		...
		$('#lbl_rowid').text("รหัส " + d.plantargetlvthree_id);
